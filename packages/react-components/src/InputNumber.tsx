...
function InputNumber ({ autoFocus,
  ...
  const _onChangeWithSi = useCallback((input: string, si: SiDef | null) => setValues(getValuesFromString(input, si, bitLength, isZeroable, maxValue)), [
    bitLength,
    isZeroable,
    maxValue
  ]);
  ...
  return (
    <Input
      ...
      value={value}
    >
      {!!si && <Dropdown className='siDropdown'
